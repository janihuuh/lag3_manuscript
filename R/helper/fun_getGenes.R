...
big_markers           <- c("CD3E", "TRAC",            ## T cell
                           "SELL", "IL7R", "LEF1", "TCF7", "CD4",
                           "CD8A", "CD8B", "GZMA", "PRF1", "GZMB", "GNLY", "CXCR3",     ## CD8+ cell
                           "IFNG", 
                           "CD27", "CD28", "IL2RA", "CCL5",             ## memory
...
#### Szabo et al Nat Comm 2019
# szabo <- fread("/Users/janihuuh/Dropbox/szabo_nat_comm_genes.txt")
# 
# treg_genes <- szabo$Treg
# nv_cm_genes <- szabo$`CD4 NV/CM`
# cd4cd8_genes <- szabo$`CD4/CD8`
# ifnresponse_genes <- szabo$`IFN Response`
# prolif_genes <- szabo$Proliferation
# cd8_cytotoxic_genes <- szabo$`CD8 Cytotoxic`
# cd8_cytolytic_genes <- szabo$`CD8 Cytokine`
# 
# szabo_markers        <- list(treg_genes, nv_cm_genes, cd4cd8_genes, ifnresponse_genes, prolif_genes, cd8_cytotoxic_genes, cd8_cytolytic_genes)
# names(szabo_markers) <- c("treg_genes", "nv_cm_genes", "cd4cd8_genes", "ifnresponse_genes", "prolif_genes", "cd8_cytotoxic_genes", "cd8_cytolytic_genes")
